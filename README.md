...
## Additional dataset
Apart from using the TL;DR, we will also be constructing an additional dataset that specifically [matches multiple YouTube transcripts](https://github.com/au-nlp/project-milestone-p2-group-6/blob/main/src/fetch_custom_dataset.py), merging the information into a single summary. [We will construct this dataset by having a larger language model (GPT 5.1) generate summaries that takes the nuances of each of 2-4 videos into account and puts out a wanted summary](https://github.com/au-nlp/project-milestone-p2-group-6/blob/main/src/summary_for_custom_dataset.py). We will have to pre-prompt it with the [precise instructions](https://github.com/au-nlp/project-milestone-p2-group-6/blob/main/data/instructions.txt) that will make it output the type of summary we want. In this way after using the TL;DR we will be using a kind of distilation of our summary model making it specifically good at this exact summary task. This would be good as we want the model to have some different properties like referencing which video says what and say when contradictions happen.
